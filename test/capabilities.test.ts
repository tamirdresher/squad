...
describe('canHandleIssue', () => {
  ...
  it('reports only missing capabilities', () => {
    ...
    if (!result.canHandle) {
      ...
      // browser IS available, should not be in missing
    }
  });
});
...
describe('filterByCapabilities', () => {
  ...
  it('filters correctly for GPU machine', () => {
    ...
    expect(handled.map(i => i.number)).toEqual([1, 2, 3, 4]); // GPU machine has both
    expect(skipped).toHaveLength(0);
  });

  it('filters correctly for laptop', () => {
    const { handled, skipped } = filterByCapabilities(issues, laptopMachine);
    expect(handled.map(i => i.number)).toEqual([1, 3]); // laptop has browser but not gpu
    expect(skipped).toHaveLength(2);
    expect(skipped[0].issue.number).toBe(2); // GPU task skipped
    expect(skipped[0].missing).toEqual(['gpu']);
    expect(skipped[1].issue.number).toBe(4); // Both task skipped (needs gpu)
  });
