...
Every spawn prompt includes these required header fields:
```
TEAM ROOT: {team_root}
CURRENT_DATETIME: <resolved CURRENT_DATETIME literal>
STATE_BACKEND: {state_backend}
```
When agents record timestamps in history entries or log files, they must substitute the resolved value — for example: `"timestamp": "<literal CURRENT_DATETIME value from your prompt>"`. Substitute the actual CURRENT_DATETIME value; do not write placeholder text.
...
Scribe spawn (background, always after a work batch):
```yaml
agent_type: "general-purpose"
