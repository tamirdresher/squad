...
### Squad.Agents.AI -- Auth surface security review (2026-06-02)

**Verdict:** PASS_WITH_CONDITIONS (9 mandatory security conditions).

**Top concerns:**
1. `SquadAgentOptions.Environment` dictionary is not redacted by `ToString()` or JSON serialization -- any token placed there (HMAC keys, API tokens per Data's documented usage) will leak via logging or serialization. Pre-existing gap, not introduced by proposal, but proposal's documented patterns make it exploitable.
2. `ConfigureCopilotClient` delegate (`Action<CopilotClientOptions>`) receives fully-populated options including resolved tokens and can override `Cwd` (WorkingDirectory isolation -- security-critical) and `CliPath`. Post-delegate validation required.

**UseLoggedInUser:** ALLOW with consent documentation.

**Full review:** `.squad/decisions/inbox/worf-squad-agents-ai-auth-security-review.md`