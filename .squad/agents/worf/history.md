...
## Learnings — Upstream bradygaster/squad dep audit (2026-06-15)

**npm Dependabot gap on JavaScript-first repos.** bradygaster/squad is a TypeScript/Node monorepo (22.5.0+) shipping npm packages (`@bradygaster/squad-sdk`, `@bradygaster/squad-cli`, etc.), yet `.github/dependabot.yml` configures ONLY NuGet + GitHub Actions. npm is entirely absent. Result: `@github/copilot-sdk` (core integration, 23 LOC import in squad-sdk) pinned to `^0.3.0` while latest is `1.0.1` (major jump). No Dependabot PR proposed — npm isn't monitored. Lesson for squad-squad: if `dependabot.yml` lists ecosystems, validate they match the project's primary language stack. A JavaScript monorepo with zero npm rules is a configuration bug, not a feature.

**Dependabot alerts disabled blocks security audit.** bradygaster/squad has alerts=disabled (HTTP 403 from API). Cannot enumerate CVEs by ecosystem without admin scope. When security alerts are disabled, it's often not deliberate — many orgs simply don't notice. Recommend: Squad security gate should include a check for "dependabot-alerts-enabled" (true/false) in any upstream-framework triage. If bradygaster/squad is under active maintenance, bug them to enable it.

**CI green with test failures is a merge trap.** PR #1316 (GitHub.Copilot.SDK 1.0.0 → 1.0.1) and #1317 (Microsoft.* group) both show `.NET builds ✅ SUCCESS` but `test ❌ FAILURE`. The build step passed; the test step failed. Easy to miss if reading status rollup too quickly. Lesson: when reviewing a Dependabot PR, drill into the test-failure details — build-success alone does not clear the blocker. Many teams have a bot that auto-merges green builds; this one does not, which is correct (tests are part of CI).

**"Mergeable: UNKNOWN" after 6 days is a stale marker.** PRs #1237, #1238 from 2026-06-09 have mergeable=UNKNOWN. This usually means the PR branch is behind dev (the default), or dev has push-protected rules that delay status computation. Always tag stale/UNKNOWN PRs as review priorities. bradygaster/squad may have a bot that auto-closes after N days; verify process before recommending closure.

**@modelcontextprotocol/sdk staying current is a good sign.** `^1.29.0` in squad-cli pins to latest (1.29.0). This suggests the maintainers do triage npm deps *manually* — they're just not using Dependabot to automate it. Corroborates the "npm ecosystem missing from config" diagnosis: if they cared, they'd enable Dependabot instead of hand-bumping.
**CROSS-AGENT NOTE (2026-06-15, Scribe):** npm vs NuGet confusion in dependabot audit — claimed npm package '@github/copilot-sdk' at ^0.3.0 vs latest 1.0.1, but GitHub.Copilot.SDK NuGet reality shows 1.0.0 → 1.0.1 pending (different ecosystems). LESSON: Verify package ecosystem (npm, NuGet, PyPI, etc.) before reporting version deltas; don't assume npm everywhere.
